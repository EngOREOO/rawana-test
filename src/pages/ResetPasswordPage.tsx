...
export default function ResetPasswordPage() {
  ...
  const passwordErrors = useMemo(
    () => ((submitted || passwordTouched) ? validateResetPassword(password) : []),
    [password, passwordTouched, submitted],
  );

  const confirmError = useMemo(() => {
    if (!submitted && !confirmTouched) return '';
    return validateResetConfirmPassword(password, confirmPassword);
  }, [confirmPassword, confirmTouched, password, submitted]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 px-4">
      <form className="w-full max-w-lg rounded-[60px] border-2 border-[#f2f2f2] bg-white p-6 shadow-xl" onSubmit={onSubmit}>
        ...
        <button type="submit" className="mt-6 w-full rounded-[8px] bg-[#dca126] px-4 py-2 text-white hover:bg-[#dca126]">
          Reset Password
        </button>
